...
var challenges;
//
// var privateKey = fs.readFileSync( '/etc/nginx/domain.key' );
// var certificate = fs.readFileSync( '/etc/nginx/lib/domain.crt' );

// Create and start the http server
var server = http.createServer().listen(port, function (err) {
    if (err) {
        throw err
    }
    init()
...
function onNewPlayer (data) {
    ...
    newPlayer.setName('Joueur sans nom ' + newPlayer.getNumber());
    newPlayer.setScore(0);

    // Broadcast new player to connected socket clients
    this.broadcast.emit('new player', {id: newPlayer.id, name: newPlayer.getName(), score: newPlayer.getScore()});

    // Send existing players to the new player
    var i, existingPlayer;
    for (i = 0; i < players.length; i++) {
        existingPlayer = players[i];
        this.emit('new player', {id: existingPlayer.id, name: existingPlayer.getName(), score: existingPlayer.getScore()})
    }
...
function onUpdatePlayer (data) {
    ...
    this.broadcast.emit('update player', {id: player.id, name: player.getName(), score: player.getScore()})
...
function onPlayerOptionSent (data) {
    // Find player in array
    var player = playerById(this.id);
    var opponent = playerById(data.opponentId);
    var challenge;
    var opponentOption;
    ...
    if (challenge.challengedOption !== '' && challenge.challengerOption  !== ''){
        ...
        if (winner) {
            ...
            this.broadcast.emit('update player', {id: player.id, name: player.getName(), score: player.getScore()});
            this.broadcast.emit('update player', {id: opponent.id, name: opponent.getName(), score: opponent.getScore()});
            this.emit('update player', {id: player.id, name: player.getName(), score: player.getScore()});
            this.emit('update player', {id: opponent.id, name: opponent.getName(), score: opponent.getScore()})
        }
